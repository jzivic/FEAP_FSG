...
def vremenska_analiza(vremena):
    ...
    for n_tren in range(len(vremena)):
        for simul in simulation_names:
            ...
            if vremena[n_tren] not in all_data.loc[simul].timeStep:
                continue
            ...
            def crtanje_kontura():
                ...
                plt.title(str(vremena[n_tren])+". korak")
            ...
            def debljina_stijenke():
                ...
                plt.title("Debljina stijenke: "+ str(vremena[n_tren]) + ". korak")
            ...
            def debljina_ILTa():
                ...
                plt.title("Debljina ILTa: "+ str(vremena[n_tren])+ ". korak")
                plt.ylabel("Thickness [mm]")
                plt.xlabel("Axial coordinate $z$ [mm]")
                plt.grid(which='both', linestyle='--', linewidth='0.5')
                plt.legend()
