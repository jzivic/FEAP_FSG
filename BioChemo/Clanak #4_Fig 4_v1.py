...
from user_functions.Classes_Functions import ReadRonodFile
from user_functions.Classes_Functions import ReadResY0File
from user_functions.Classes_Functions import ReadResY0ILTFile
from user_functions.Classes_Functions import ReadMiddleFile
import matplotlib.pyplot as plt

# Set working directory
# work_dir = r"C:\Users\Nino\Desktop\Nino FEAP\FEAP_model_redesign\Analize\Build test - ILT"
# work_dir = work_dir + r"\172 - Biochemo fuziformna 14\RESULTS"

work_dir = "//home/josip/feap/FSG/automatizacija_33_N_AVERAGE/radial/radial_tawss_40_d02"
...
ronod_axial_timesteps = [200,300,400]
...
if plot_res_y0_inner or plot_res_y0_outer:
...
if plot_ronod_node:
    ...
    for k in range(num_of_files):
        for j in range(len(ronod_nodes)):
            ...
            if j == 0:
                p2.plot(time, M_elastin_s, color=c4, label=r"Elastin in section A", linestyle='-')
                p2.plot(time, M_coll_s, color=c4, label=r"Collagen in section A", linestyle='--')
                p2.plot(time, M_SMC_s, color=c4, label=r"SMC in section A", linestyle=':')
            if j == 1:
...
if plot_ronod_axial:
    ...
    for k in range(num_of_files):
        for i in range(len(ronod_axial_timesteps)):
            ...
            if i == 0:
                p1.plot(M_elastin_s, z, color=c1, label=r"Elastin at $\tau = 2000$ days", linestyle='-')
                p1.plot(M_coll_s, z, color=c3, label=r"Collagen at $\tau = 2000$ days", linestyle='-')
                p1.plot(M_SMC_s, z, color=c2, label=r"SMC at $\tau = 2000$ days", linestyle='-')
            if i == 1:
                p1.plot(M_elastin_s, z, color=c1, label=r"Elastin at $\tau = 3000$ days", linestyle='--')
                p1.plot(M_coll_s, z, color=c3, label=r"Collagen at $\tau = 3000$ days", linestyle='--')
                p1.plot(M_SMC_s, z, color=c2, label=r"SMC at $\tau = 3000$ days", linestyle='--')
            if i == 2:
                p1.plot(M_elastin_s, z, color=c1, label=r"Elastin at $\tau = 4000$ days", linestyle='-.')
                p1.plot(M_coll_s, z, color=c3, label=r"Collagen at $\tau = 4000$ days", linestyle='-.')
                p1.plot(M_SMC_s, z, color=c2, label=r"SMC at $\tau = 4000$ days", linestyle='-.')

    # p1.set_xlim(0,10000)
    p1.set_ylim(75,275)
    p1.set_title("(a)")
    p1.set_xlabel("Normalized mass")
    p1.set_ylabel(r"Axial coordinate $z$ [mm]")
    p1.grid(which='both', linestyle='--', linewidth='0.5')
    p1.legend(facecolor='w', framealpha=1.0, bbox_to_anchor=(1.0, -0.1))
    p1.spines['top'].set_visible(False)
...
# plt.savefig(fig_name_fig3, bbox_inches='tight')
# plt.show()
# plt.close(12)
