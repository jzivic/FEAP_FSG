     ...
     &                 M_MMP_history(num_stored_steps), mass(25)
...
c     Current radii
      call outer_radius(hr(up(36)),hr(up(15)),M_elas_real)
...
c       Calculate total surface od vasa vasorum
        ...
        MEDP_tot = bio_EDP_ratio * (M_elas_real(1) - M_elas_real(2))
        AVV_tot = bio_AVV_0 + bio_KVV_EDP * MEDP_tot    ! K^VV_EDP - correlation factor [mm^2_VV/g_M^EDP_tot]  
        
c       Calculate parameters A and B
        A_elas = (bio_KN_elas * MN_tot - bio_KVV_elas * AVV_tot)
     &           / dlog(rl/ro)
        A_MMP = (bio_KN_MMP * MN_tot - bio_KVV_MMP * AVV_tot)
     &           /dlog(rl/ro)
        B_elas = (bio_KVV_elas * AVV_tot * dlog(rl) - 
     &            bio_KN_elas * MN_tot * dlog(ro)) / dlog(rl/ro)
        B_MMP = (bio_KVV_MMP * AVV_tot * dlog(rl) - 
     &            bio_KN_MMP * MN_tot * dlog(ro)) / dlog(rl/ro)
...
c       Calculate concentrations
        M_elas = A_elas * dlog(rip) + B_elas
        M_MMP = A_MMP * dlog(rip) + B_MMP
        
        bio2_enzyme_flag = .true.
