...
c     User variables
     ...
     &          sigma(3,3), ff(3,3)
      ...
      ! Mass calculation
      real*8    mass(25), production(num_stored_steps,5), 
...
c     CALCULATE CONSTITUENTS MASS
	  ...
	  ! -- First newton and augmented iteratio. --
        ...
        if (timestep.eq.1) then   ! ttim = 0
          ...
          ! call calculate_element_real_mass(mass(23),n,hr(np(43)),mr(np(33)))
        endif
...
!     &     modfeap_number.ne.0) then
        if(isw.eq.3.and.ilt_biochemo_flag.ge.2) then
