...
c     Save values from input file
      ...
      do while (.not.pcomp(text(1),'    ',4))
        ...
        elseif (pcomp(text(1),'flg4',4)) then 
            ...
            ilt_F_identity_flag =   td(12)
            if (ilt_biochemo_flag.gt.1) then
              geometry_type = 3
            endif
        ...
        elseif (pcomp(text(1),'num3',4)) then   
            ...
            no_follower_press_mat = td(2)
            exit_timestep =         td(3)
            if (exit_timestep.eq.0) then
              exit_timestep = 1000
            endif
            sleep_time_diff =       nint(td(4))
