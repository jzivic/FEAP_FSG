...
c      Outputs:
     ...
     &          nd,nd1,nd2,nd3,nd4,nd5,nd6,nd7,nd8,nd9,id1,id2,id3,
     &          npoint,nd_left,nd_right,nd_down,nd_up,nd_base,
     &          n1,n2,n3,n4,press_elem_number,mp1,mp2,mp3,mp4,side,
     &          updown,nd0,ndA,ndB,ndC,ndE,el,imn,nd_bellow,
     &          nd_bel_down,nd_bel_left,nd_bel_right,nd_bel_up,
     &          nd_cor1,nd_cor2,nd_cor3,nd_cor4
...
c     Write new pressure elements in file
     ...
     &                        nd1,nd2,nd3,nd4,zero,zero,zero,zero
      ...
      elseif(move_pressure.eq.3) then
        
        ! elementi tlaka ispod tromba
        do j = 1,nnz-1
          do i = 1,nnt-1
            if(elemmap(i,j,1).ne.0) then
              nd1 = nodemap(2,i,j,1)
              nd2 = nodemap(2,i,j+1,1)
              nd3 = nodemap(2,i+1,j+1,1)
              nd4 = nodemap(2,i+1,j,1)
              if(.not.press_elem_title) then
                write(45,*) 'ELEMents'
                press_elem_title = .true.
              endif 
              write(45,1003) press_elem_number,ndgen,
     &         no_follower_press_mat,nd1,nd2,nd3,nd4,zero,zero,zero,zero
              press_elem_number = press_elem_number+1
            endif
          enddo
        enddo
...
c     Write mesh-boun file (43) and mesh-angl file (42)
     ...
     &             (nd_up.eq.0.or.nd_down.eq.0)) link_z = .true.
      ...
      elseif (ilt_boundary_flag.eq.3) then
      ...
      else   ! (ilt_boundary_flag.eq.4) then
              ...
              if (k.gt.1.and.nd1.ne.0) then
                ...
                elseif (i.eq.1.or.i.eq.nnt) then  ! bocne linije (linkanje samo u z smjeru)
                  ...
                  if (nd_up.eq.0.or.nd_down.eq.0) then
                    write(44,1006) nd1,nd_base, 0,0, 1, 1, 0
                    if (k.ge.3) then
                      nd_bellow = nodemap(1,i,j,k-1)
                      nd_bel_up = nodemap(1,i,j+1,k-1)
                      nd_bel_down = nodemap(1,i,j-1,k-1)
                      if (nd_bel_up.ne.0.and.nd_bel_down.ne.0) then
                        write(44,1006) nd1,nd_bellow, 0,0, 1, 1, 0
                      endif
                    endif
                  endif
                elseif (j.eq.1.or.j.eq.nnz) then  ! gornja i donja linija (linkanje samo u theta smjeru)
                  ...
                  if (nd_left.eq.0.or.nd_right.eq.0) then
                    ...
                    if (k.ge.3) then
                      nd_bellow = nodemap(1,i,j,k-1)
                      nd_bel_left = nodemap(1,i-1,j,k-1)
                      nd_bel_right = nodemap(1,i+1,j,k-1)
                      if (nd_bel_left.ne.0.and.nd_bel_right.ne.0) then
                        write(44,1006) nd1,nd_bellow, 0,0, 1, 0, 1
                      endif
                    endif
                  endif
                else
     ...
     &                nd_right.eq.0) then
                  ...
                  if (k.ge.3) then
                    nd_cor1 = nodemap(1,i-1,j-1,k)
                    nd_cor2 = nodemap(1,i+1,j-1,k)
                    nd_cor3 = nodemap(1,i+1,j+1,k)
                    nd_cor4 = nodemap(1,i-1,j+1,k)
                    nd_bellow = nodemap(1,i,j,k-1)
                    nd_bel_up = nodemap(1,i,j+1,k-1)
                    nd_bel_down = nodemap(1,i,j-1,k-1)
                    nd_bel_left = nodemap(1,i-1,j,k-1)
                    nd_bel_right = nodemap(1,i+1,j,k-1)
                    if(nd_cor1.eq.0.or.nd_cor2.eq.0.or.nd_cor3.eq.0.or.
     &                   nd_cor4.eq.0) then
                      if (nd_bel_up.ne.0.and.nd_bel_down.ne.0.and.
     &                   nd_bel_left.ne.0.and.nd_bel_right.ne.0) then
                        write(44,1006) nd1,nd_bellow, 0,0, 1, 0, 0
                      elseif (nd_bel_up.ne.0.and.nd_bel_down.ne.0) then
                        write(44,1006) nd1,nd_bellow, 0,0, 1, 1, 0
                      elseif(nd_bel_left.ne.0.and.nd_bel_right.ne.0)then
                        write(44,1006) nd1,nd_bellow, 0,0, 1, 0, 1
                      endif
                    endif
                  endif
      ...
      if (ilt_biochemo_flag.eq.3) then
        ...
        do i = 1,nnt      ! razina
          do j = 1,nnz      ! theta smjer
            do k = 1,nnr-1    ! z smjer
...
c     Based on number of new elements change numnp and numel
      ...
      num_el_ilt = element_number - wall_numnl_with_pr - 1
...
1003  format(i5,i3,i3,i8,i7,i7,i7,i7,i7,i7,i7)
