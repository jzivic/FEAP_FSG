     ...
     &          point_counter, tawws_line_number
...
c     Chech for results files
      ...
      do while (waiting)
        ...
        inquire(file='koordinate',exist=exist_kordinate)
        
        ! ako ne postoje ispisi CFDerrorA
        
        if (exist_tawss.and.exist_kordinate) then
          waiting = .false.
        else
          write(*,*)
...
c     Read results files
      if (luminal_surface_flag.ge.10.and.luminal_surface_flag.le.15)then
...
10          call vinput(line(2:nl-1),nl-2,coor,3) 
        ...
        do while (reading)
          read(51,'(a80)') line
          if (pcomp(line(1:2),'(',1)) then
            tawws_line_number = tawws_line_number + 1 
          endif
          ! End of results
          if (pcomp(line(1:2),')',1)) then
            reading = .false.
     ...
     &          luminal_surface_flag.eq.15) then  
          ...
          do j = 2,nnz-1
            ...
            ! Loop over "kordinate" file to find closest upper and lower point
...
c     Open file for writing feap CFD values
      if (modfeap_number.eq.1) then
        open(55,file='TAWSS_values_feap', status='replace',
     &       action='write', form='formatted')
        write(55,1005)
        write(55,1013)
      else
        open(55,file='TAWSS_values_feap', status='old',
     &       action='write', form='formatted')
      endif
...
1005  format(//1x 'TAWSS results in lumen noded - interpolated from CFD
     & results')
